...
namespace thinkyoung {
    namespace client {
        ...
        namespace detail {
            ...
            void ClientImpl::start_delegate_loop() {
                ...
                if (!_time_discontinuity_connection.connected())
                    _time_discontinuity_connection = thinkyoung::blockchain::time_discontinuity_signal.connect([=]() {
                    reschedule_delegate_loop();
                });
                _delegate_loop_complete = fc::async([=]() {
                    delegate_loop();
                }, "delegate_loop");
            }
            ...
            void ClientImpl::delegate_loop() {
                ...
                if (!_delegate_loop_complete.canceled())
                    _delegate_loop_complete = fc::schedule([=]() {
                    delegate_loop();
                }, scheduled_time, "delegate_loop");
            }
            ...
            void ClientImpl::start_rebroadcast_pending_loop() {
                if (!_rebroadcast_pending_loop_done.valid() || _rebroadcast_pending_loop_done.ready())
                    _rebroadcast_pending_loop_done = fc::schedule([=]() {
                    rebroadcast_pending_loop();
                },
            ...
            void ClientImpl::start_fork_update_time_loop() {
                if (!_fork_update_loop_done.valid() || _fork_update_loop_done.ready())
                    _fork_update_loop_done = fc::schedule([=]() {
                    fork_update_time_loop();
                },
            ...
            void ClientImpl::rebroadcast_pending_loop() {
                ...
                struct checker {
                    ...
                    checker(bool& var) : var(var) {
                        if (var) elog("Checker failure!");
        ...
        thinkyoung::rpc::RpcServerPtr Client::get_rpc_server()const {
            return my->_rpc_server;
            }
        thinkyoung::net::NodePtr Client::get_node()const {
            return my->_p2p_node;
        }
        ...
        void Client::configure_from_command_line(int argc, char** argv) {
            ...
            if (option_variables.count("growl") || my->_config.growl_notify_endpoint) {
                ...
                my->_blocks_too_old_monitor_done = fc::schedule([=]() {
                    my->blocks_too_old_monitor_task();
                },
                    fc::time_point::now() + fc::seconds(ALP_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
                    "block_monitor_task");
            }
