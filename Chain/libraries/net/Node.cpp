...
FC_REFLECT(thinkyoung::net::detail::NodeConfiguration, (listen_endpoint)
    ...
    namespace net {
        namespace detail {
            ...
            struct PrioritizedItemId
            {
                ...
                {
                    static_assert(thinkyoung::client::block_message_type > thinkyoung::client::trx_message_type,
...
#define BLACKLIST_DATABASE_FILENAME      "blacklist.leveldb"
                ...
                thinkyoung::db::LevelMap<std::string, fc::ip::endpoint> _black_list_db;
...
#define MAXIMUM_NUMBER_OF_BLOCKS_TO_PREFETCH (10 * MAXIMUM_NUMBER_OF_BLOCKS_TO_HANDLE_AT_ONE_TIME)
            ...
            NodeImpl::NodeImpl(const std::string& user_agent) :
                ...
                _desired_number_of_connections(ALP_NET_DEFAULT_DESIRED_CONNECTIONS),
                _maximum_number_of_connections(ALP_NET_DEFAULT_MAX_CONNECTIONS),
                _peer_connection_retry_timeout(ALP_NET_DEFAULT_PEER_CONNECTION_RETRY_TIME),
                _peer_inactivity_timeout(ALP_NET_PEER_HANDSHAKE_INACTIVITY_TIMEOUT),
                ...
                _maximum_blocks_per_peer_during_syncing(ALP_NET_MAX_BLOCKS_PER_PEER_DURING_SYNCING),
                _sync_item_type(0)
            {
            ...
            void NodeImpl::p2p_network_connect_loop()
            {
                ...
                while (!_p2p_network_connect_loop_done.canceled())
                {
                    try
                    {
                        ...
                        try
                        {
                            _retrigger_connect_loop_promise = fc::promise<void>::ptr(new fc::promise<void>("thinkyoung::net::retrigger_connect_loop"));
                            if (is_wanting_new_connections() || !_add_once_node_list.empty())
                            {
                                ...
                                _retrigger_connect_loop_promise->wait_until(fc::time_point::now() + fc::seconds(ALP_PEER_DATABASE_RETRY_DELAY));
                            }
                            else
                            {
            ...
            void NodeImpl::request_sync_item_from_peer(const PeerConnectionPtr& peer, const ItemHashType& item_to_request)
            {
                ...
                ItemId item_id_to_request(thinkyoung::client::block_message_type, item_to_request);
            ...
            void NodeImpl::request_sync_items_from_peer(const PeerConnectionPtr& peer, const std::vector<ItemHashType>& items_to_request)
            {
                ...
                peer->send_message(FetchItemsMessage(thinkyoung::client::block_message_type, items_to_request));
            }
            ...
            void NodeImpl::fetch_sync_items_loop()
            {
                ...
                while (!_fetch_sync_items_loop_done.canceled())
                {
                    ...
                    if (!_sync_items_to_fetch_updated)
                    {
                        ...
                        _retrigger_fetch_sync_items_loop_promise = fc::promise<void>::ptr(new fc::promise<void>("thinkyoung::net::retrigger_fetch_sync_items_loop"));
            ...
            void NodeImpl::fetch_items_loop()
            {
                ...
                while (!_fetch_item_loop_done.canceled())
                {
                    ...
                    for (auto item_iter = _items_to_fetch.begin(); item_iter != _items_to_fetch.end();)
                    {
                        ...
                        else
                        {
                            ...
                            for (auto peer_iter = items_by_peer.get<requested_item_count_index>().begin(); peer_iter != items_by_peer.get<requested_item_count_index>().end(); ++peer_iter)
                            {
                                ...
                                if (peer_iter->item_ids.size() < ALP_NET_MAX_ITEMS_PER_PEER_DURING_NORMAL_OPERATION &&
                                    peer->inventory_peer_advertised_to_us.find(item_iter->item) != peer->inventory_peer_advertised_to_us.end())
                                {
                                    if (item_iter->item.item_type == thinkyoung::net::trx_message_type && peer->is_transaction_fetching_inhibited())
                    ...
                    {
                        _retrigger_fetch_item_loop_promise = fc::promise<void>::ptr(new fc::promise<void>("thinkyoung::net::retrigger_fetch_item_loop"));
            ...
            void NodeImpl::advertise_inventory_loop()
            {
                ...
                while (!_advertise_inventory_loop_done.canceled())
                {
                    ...
                    {
                        _retrigger_advertise_inventory_loop_promise = fc::promise<void>::ptr(new fc::promise<void>("thinkyoung::net::retrigger_advertise_inventory_loop"));
            ...
            void NodeImpl::terminate_inactive_connections_loop()
            {
                ...
                uint32_t active_disconnect_timeout = std::max<uint32_t>(5 * ALP_BLOCKCHAIN_BLOCK_INTERVAL_SEC / 2, 30);
                uint32_t active_send_keepalive_timeount = std::max<uint32_t>(active_disconnect_timeout / 2, 11);
                uint32_t active_ignored_request_timeount = std::max<uint32_t>(ALP_BLOCKCHAIN_BLOCK_INTERVAL_SEC / 4, 10);
                ...
                fc::time_point closing_disconnect_threshold = fc::time_point::now() - fc::seconds(ALP_NET_PEER_DISCONNECT_TIMEOUT);
                for (const PeerConnectionPtr& closing_peer : _closing_connections)
                    ...
                    {
                        // we asked this peer to close their connectoin to us at least ALP_NET_PEER_DISCONNECT_TIMEOUT
                ...
                if (!_node_is_shutting_down && !_terminate_inactive_connections_loop_done.canceled())
                    ...
                    fc::time_point::now() + fc::seconds(ALP_NET_PEER_HANDSHAKE_INACTIVITY_TIMEOUT / 2),
            ...
            bool NodeImpl::merge_address_info_with_potential_peer_database(const std::vector<AddressInfo>& addresses)
            {
                ...
                {
                    if (address.firewalled == thinkyoung::net::FirewalledState::not_firewalled)
                    {
            ...
            void NodeImpl::on_message(PeerConnection* originating_peer, const Message& received_message)
            {
                ...
                dlog("handling message ${type} ${hash} size ${size} from peer ${endpoint}",
                    ("type", thinkyoung::net::CoreMessageTypeEnum(received_message.msg_type))("hash", message_hash)
            ...
            fc::variant_object NodeImpl::generate_hello_user_data()
            {
                ...
                user_data["thinkyoung_git_revision_sha"] = thinkyoung::utilities::git_revision_sha;
                user_data["thinkyoung_git_revision_unix_timestamp"] = thinkyoung::utilities::git_revision_unix_timestamp;
                ...
                user_data["delegate_num"] = ALP_BLOCKCHAIN_NUM_DELEGATES;
            ...
            void NodeImpl::parse_hello_user_data_for_peer(PeerConnection* originating_peer, const fc::variant_object& user_data)
            {
                ...
                if (user_data.contains("thinkyoung_git_revision_sha"))
                    originating_peer->thinkyoung_git_revision_sha = user_data["thinkyoung_git_revision_sha"].as_string();
                if (user_data.contains("thinkyoung_git_revision_unix_timestamp"))
                    originating_peer->thinkyoung_git_revision_unix_timestamp = fc::time_point_sec(user_data["thinkyoung_git_revision_unix_timestamp"].as<uint32_t>());
            ...
            void NodeImpl::on_hello_message(PeerConnection* originating_peer, const HelloMessage& hello_message_received)
            {
                ...
                if (originating_peer->last_known_fork_block_number == 0 &&
                    originating_peer->thinkyoung_git_revision_unix_timestamp)
                {
                    uint32_t unix_timestamp = originating_peer->thinkyoung_git_revision_unix_timestamp->sec_since_epoch();
                ...
                if (originating_peer->their_state == PeerConnection::TheirConnectionState::just_connected)
                {
                    ...
                    if (delegate_num != ALP_BLOCKCHAIN_NUM_DELEGATES)
                    {
                        wlog("Invalid client");
                    ...
                    if (originating_peer->last_known_fork_block_number != 0)
                    {
                        ...
                        if (next_fork_block_number != 0)
                        {
                            ...
                            if (next_fork_block_number < head_block_num)
                            {
                                ...
                                ulog("Rejecting connection from peer because their version is too old.  Their version date: ${date}", ("date", originating_peer->thinkyoung_git_revision_unix_timestamp));
#endif
            ...
            void NodeImpl::on_address_message(PeerConnection* originating_peer, const AddressMessage& address_message_received)
            {
                ...
                std::vector<thinkyoung::net::AddressInfo> updated_addresses = address_message_received.addresses;
                ...
                const BlockchainItemIdsInventoryMessage& blockchain_item_ids_inventory_message_received)
            {
                ...
                if (originating_peer->item_ids_requested_from_peer)
                {
                    ...
                    fc::time_point_sec minimum_time_of_last_offered_block =
                        ...
                        originating_peer->number_of_unfetched_item_ids * ALP_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    if (minimum_time_of_last_offered_block > _delegate->get_blockchain_now() + ALP_NET_FUTURE_SYNC_BLOCKS_GRACE_PERIOD_SEC)
                    {
                    ...
                    if (blockchain_item_ids_inventory_message_received.total_remaining_item_count != 0)
                    {
                        ...
                        if (originating_peer->ids_of_items_to_get.size() > ALP_NET_MIN_BLOCK_IDS_TO_PREFETCH)
                        {
            ...
            void NodeImpl::on_item_ids_inventory_message(PeerConnection* originating_peer, const ItemIdsInventoryMessage& item_ids_inventory_message_received)
            {
                ...
                for (const ItemHashType& item_hash : item_ids_inventory_message_received.item_hashes_available)
                {
                    ...
                    if (!we_advertised_this_item_to_a_peer)
                    {
                        ...
                        if ((item_ids_inventory_message_received.item_type == thinkyoung::client::trx_message_type &&
...
#ifdef ENABLE_DEBUG_ULOGS
                                    ulog("Disconnecting from peer during sync because their version is too old.  Their version date: ${date}", ("date", peer->thinkyoung_git_revision_unix_timestamp));
#endif
                ...
                const GetCurrentConnectionsRequestMessage& get_current_connections_request_message_received)
            {
                ...
                for (const PeerConnectionPtr& peer : _active_connections)
                {
                    ...
                    fc::mutable_variant_object user_data;
                    if (peer->thinkyoung_git_revision_sha)
                        user_data["thinkyoung_git_revision_sha"] = *peer->thinkyoung_git_revision_sha;
                    if (peer->thinkyoung_git_revision_unix_timestamp)
                        user_data["thinkyoung_git_revision_unix_timestamp"] = *peer->thinkyoung_git_revision_unix_timestamp;
                ...
                const Message& message_to_process, const MessageHashType& message_hash)
            {
                ...
                else
                {
                    ...
                    catch (const insufficient_relay_fee&)
                    {
                        ...
                        assert(message_to_process.msg_type == thinkyoung::client::trx_message_type); // we only support throttling transactions.
                        if (message_to_process.msg_type == thinkyoung::client::trx_message_type)
                            originating_peer->transaction_fetching_inhibited_until = fc::time_point::now() + fc::seconds(ALP_NET_INSUFFICIENT_RELAY_FEE_PENALTY_SEC);
                        return;
                    }
                    catch (const fc::canceled_exception&)
                    {
            ...
            void NodeImpl::load_configuration(const fc::path& configuration_directory)
            {
...
#ifdef ALP_TEST_NETWORK
                    //uint32_t port = ALP_NET_TEST_P2P_PORT + ALP_TEST_NETWORK_VERSION;
                    uint32_t port = ALP_NET_TEST_P2P_PORT;
#else
                    uint32_t port = ALP_NET_DEFAULT_P2P_PORT;
#endif
                ...
                try
                {
                    ...
                    thinkyoung::db::LevelMap<std::string, fc::ip::endpoint>::iterator  it = _black_list_db.begin();
                    while (it.valid())
                    {
            ...
            std::vector<PeerStatus> NodeImpl::get_connected_peers() const
            {
                ...
                for (const PeerConnectionPtr& peer : _active_connections)
                {
                    ...
                    if (peer->thinkyoung_git_revision_sha)
                    {
                        std::string revision_string = *peer->thinkyoung_git_revision_sha;
                        if (*peer->thinkyoung_git_revision_sha == thinkyoung::utilities::git_revision_sha)
                            revision_string += " (same as ours)";
                        else
                            revision_string += " (different from ours)";
                        peer_details["thinkyoung_git_revision_sha"] = revision_string;

                    }
                    if (peer->thinkyoung_git_revision_unix_timestamp)
                    {
                        peer_details["thinkyoung_git_revision_unix_timestamp"] = *peer->thinkyoung_git_revision_unix_timestamp;
                        std::string age_string = fc::get_approximate_relative_time_string(*peer->thinkyoung_git_revision_unix_timestamp);
                        if (*peer->thinkyoung_git_revision_unix_timestamp == fc::time_point_sec(thinkyoung::utilities::git_revision_unix_timestamp))
                            age_string += " (same as ours)";
                        else if (*peer->thinkyoung_git_revision_unix_timestamp > fc::time_point_sec(thinkyoung::utilities::git_revision_unix_timestamp))
                            age_string += " (newer than ours)";
                        else
                            age_string += " (older than ours)";
                        peer_details["thinkyoung_git_revision_age"] = age_string;
                    }
            ...
            void NodeImpl::broadcast(const Message& item_to_broadcast, const MessagePropagationData& propagation_data)
            {
                ...
                if (item_to_broadcast.msg_type == thinkyoung::client::block_message_type)
                {
                    thinkyoung::client::BlockMessage block_message_to_broadcast = item_to_broadcast.as<thinkyoung::client::BlockMessage>();
            ...
            MessagePropagationData NodeImpl::get_transaction_propagation_data(const thinkyoung::blockchain::TransactionIdType& transaction_id)
            {
                VERIFY_CORRECT_THREAD();
                return _message_cache.get_message_propagation_data(transaction_id);
            }

            MessagePropagationData NodeImpl::get_block_propagation_data(const thinkyoung::blockchain::BlockIdType& block_id)
            {
                VERIFY_CORRECT_THREAD();
...
# define INVOKE_IN_IMPL(method_name, ...) \
    return my->method_name(__VA_ARGS__)
        ...
        MessagePropagationData Node::get_transaction_propagation_data(const thinkyoung::blockchain::TransactionIdType& transaction_id)
        {
            INVOKE_IN_IMPL(get_transaction_propagation_data, transaction_id);
        }

        MessagePropagationData Node::get_block_propagation_data(const thinkyoung::blockchain::BlockIdType& block_id)
        {
